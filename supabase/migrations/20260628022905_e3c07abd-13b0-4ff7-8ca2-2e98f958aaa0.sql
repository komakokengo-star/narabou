CREATE OR REPLACE FUNCTION public.is_request_worker(_request_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.is_request_customer(_request_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.is_match_worker(_match_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.can_access_checkin_match(_match_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
USING (
  ...
  OR (public.has_role(auth.uid(), 'worker') AND public.is_request_worker(id, auth.uid()))
);
...
USING (
  auth.uid() = customer_id
  OR public.has_role(auth.uid(), 'admin')
  OR public.is_request_worker(id, auth.uid())
)
WITH CHECK (
  auth.uid() = customer_id
  OR public.has_role(auth.uid(), 'admin')
...
USING (
  auth.uid() = worker_id
  OR public.has_role(auth.uid(), 'admin')
  OR public.is_request_customer(request_id, auth.uid())
);
...
TO authenticated
USING (
  public.can_access_checkin_match(match_id, auth.uid())
  OR public.has_role(auth.uid(), 'admin')
);
...
FOR INSERT
TO authenticated
WITH CHECK (public.is_match_worker(match_id, auth.uid()));